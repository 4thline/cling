...
public class LightActivity extends Activity implements PropertyChangeListener {
    ...
    private P2PState wifiP2pState = P2PState.ChannelDisconnected;
    private BroadcastReceiver wifiP2pBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            publishState(wifiP2pInfo.groupFormed && wifiP2pInfo.isGroupOwner ? P2PState.GroupCreated : wifiP2pState);
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        publishState(P2PState.ChannelConnected);

        registerReceiver(wifiP2pBroadcastReceiver, new IntentFilter(WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION));
        ...
        p2pChannel = wifiP2pManager.initialize(this, getMainLooper(), new WifiP2pManager.ChannelListener() {
            ...
            public void onChannelDisconnected() {
                publishState(P2PState.ChannelDisconnected);
            }
        });
    ...
    private void publishState(final P2PState state) {
        wifiP2pState = state;
        if (getActionBar() != null)
            getActionBar().setSubtitle(state.name());

        invalidateOptionsMenu();

        log.log(Level.FINEST, "State change to : " + state.name());
    }
        ...
        if (wifiP2pState.ordinal() >= P2PState.ChannelConnected.ordinal()) {
            wifiP2pManager.removeGroup(p2pChannel, new WifiP2pManager.ActionListener() {
                ...
                public void onSuccess() {
                    log.log(Level.FINEST, "Destroyed the group successfully.");
                }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        menu.add(0, 10, 0, R.string.resetLog);
        menu.add(0, 11, 0, R.string.wifiDirectBrowser);
        ...
        if (wifiP2pState == P2PState.ChannelConnected) {
            menu.add(0, 2, 0, R.string.toggleWifiDirectGroup);
        } else if (wifiP2pState == P2PState.GroupCreated) {
            menu.add(0, 3, 0, R.string.toggleWifiDirectGroup);
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case 2:
                // TODO: Main Looper?
                wifiP2pManager.createGroup(p2pChannel, new WifiP2pManager.ActionListener() {
                    @Override
                    public void onSuccess() {
                        publishState(P2PState.GroupCreated);
                        final WifiP2pServiceInfo info = WifiP2pDnsSdServiceInfo.newInstance(P2pConstants.WIFI_P2P_INSTANCE_NAME, P2pConstants.WIFI_P2P_SERVICE_TYPE, null);
                        wifiP2pManager.addLocalService(p2pChannel, info, new WifiP2pManager.ActionListener() {
                            @Override
                            public void onSuccess() {
                                publishState(P2PState.ServiceCreated);
                            }

                            @Override
                            public void onFailure(final int i) {
                                publishState(P2PState.ServiceCreationFailed);
                            }
                        });
                    }
                    ...
                    @Override
                    public void onFailure(final int reason) {
                        publishState(P2PState.GroupCreationFailed);
                    }
                });
            ...
            case 10:
                ...
                break;
            case 11:
                startActivity(new Intent(this, WifiDirectBrowserActivity.class));
                break;
        }
        return false;
    }
