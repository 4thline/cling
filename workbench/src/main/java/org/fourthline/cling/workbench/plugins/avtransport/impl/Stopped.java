 ...
 */
public class Stopped extends InstanceViewState {

    public Stopped(InstanceViewImpl view) {
        super(view);
    }
    ...
    public void onEntry() {
        new ViewUpdate() {
            protected void run(InstanceViewImpl view) {
                view.getPlayerPanel().setBorder(BorderFactory.createTitledBorder("STOPPED"));
                view.getPlayerPanel().setAllButtons(false);
                view.getPlayerPanel().getPlayButton().setEnabled(true);
                view.getProgressPanel().setProgress(null);
            }
        };
    }
